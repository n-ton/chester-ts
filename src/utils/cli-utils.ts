...
import { capsConfig } from '../config/driver/capabilities-config'
import { selenoidConfig } from '../config/driver/selenoid-config'
import IEnvConfig from '../config/env/i-env-config'
import { envConfig } from '../config/env/env-config'
import IReporterConfig from '../config/mocha/reporting/i-reporter-config'
import { reporterConfig } from '../config/mocha/reporting/reporter-config'
...
import { specsConfig } from '../../test/specs/specs-config'
...
export default class CliUtils {
  ...
  static getSpecFileFromCli(): string {
    const specFile = this.getParam(this.SPEC_FILE_PARAM)
    if (isUndefined(specFile)) {
      console.warn(
        '--specFile has not been provided in CLI'
      )
      return 'undefined'
    }
    console.info(`Applied specFile: ${specFile}`)
    return specFile
  }
  ...
  static getEnvConfigFromCli(): IEnvConfig {
    ...
    let config: IEnvConfig | undefined
    if (isUndefined(value)) {
  ...
  static getReporterConfigFromCli(): IReporterConfig {
    ...
    let config : IReporterConfig | undefined
    if (isUndefined(value)) {
      console.warn(
  ...
  static getDebugConfigFromCli(): IDebugConfig {
    ...
    let config: IDebugConfig | undefined
    if (isUndefined(value)) {
      console.warn(
